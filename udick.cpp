...
/*          Context data pool        	  				   */
/*---------------------------------------------------------*/
context_data_t context_pool[MAXPROC];
...
//      < initialize process stack >
    ...
    vdes[p].context = m_process;
    Serial.println("m_process");
    Serial.println((int)m_process);
...
//    noInterrupts();// < disable interrupts >*/
    ...
    Serial.print("vdes[pexe].context = ");//Serial.println(vdes[pexe].context ? "Initialized" : "None");
    Serial.println("carga pxcurrentContext");
    pxCurrentContext = vdes[pexe].context;
    Serial.println("cargado pxcurrentContext");
...
//Serial.print("Time unit=");
    //Serial.println(tick);
    Serial.println((int)pxCurrentContext);
    delay(300);
    SAVE_CONTEXT();
...
{
    Serial.println("load_context - entering...");
    pxCurrentContext = vdes[pexe].context;
    LOAD_CONTEXT();
//    interrupts();// < enable interrupts >
	...
	Serial.println("load_context - about to jump");
	asm volatile ( "ret" );
}
...
//      < initialize process stack >
    //vdes[p].context = (Context)context_pool[p].data;
    vdes[p].context = addr;
    Serial.println((int)vdes[p].context);
    interrupts(); //< enable cpu interrupts >
